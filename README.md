...
- ~~**Port d'écoute**~~ **résolu** : `src/config/env.js#boot` retombe automatiquement sur la variable générique `PORT` si `WEB_PORT` n'est pas définie — couvre le cas où blitz.cloud (ou tout autre PaaS) injecte son propre nom de variable plutôt que le nôtre. Suspecté comme cause d'un premier déploiement resté bloqué indéfiniment sur "Starting it up" (la sonde de disponibilité de la plateforme visait probablement un port que le process n'écoutait jamais) ; vérifié par un build + run réels en local avec uniquement `PORT` défini (pas `WEB_PORT`) — le process s'y lie correctement.
