...
| Rév. m (2026-09-17, A32) — Lien "Panel admin" sur le tableau de bord, conditionné à `req.caps.size > 0` — aucun mécanisme de bootstrap self-service pour le premier `proprietaire` (gap noté, non résolu à ce stade) — cf. `1-CheckList.md` A32
| Rév. q (2026-09-17, E10) — Bootstrap du premier admin tranché, pas de code à écrire : Arthus devient `proprietaire` en prod via le même `INSERT` SQL manuel que celui utilisé en dev pour `arde_ass`, puis attribue le rôle admin à Le_Club depuis le panel (`rbac.assignRole`, fonctionnel depuis A27) — cf. `1-CheckList.md` E10
| Rév. r (2026-09-17, A35) — Thème CSS unique (`web/public/style.css`, dark Discord/Disboard), icônes de serveur + avatars Discord (CDN, `guildIconHtml`/`userAvatarHtml` dans `render.js`), `guilds.icon_hash` enfin peuplé (`guildWatcher.js`, `oauth.js`). Quatre bugs trouvés en le faisant : violation CSP silencieuse (`style="display:inline"` × 15, `style-src 'self'`), `icon_hash` jamais écrit nulle part depuis sa création en Phase I, crash `BigInt()` sur un `discord_id` de fixture non-snowflake, cache `immutable: 30j` sans cache-buster sur `style.css` — cf. `1-CheckList.md` A35
| Rév. s (2026-09-20, A36) — Cartes façon "Découvrir" Discord (`render.js:cardHtml`) partout, y compris le tableau de bord (auparavant seul à garder l'ancien style en liste) ; chargement des vraies bannières Discord serveur/utilisateur (migration 008, `banner_hash`/`splash_hash`) ; section "Avis" enfin affichée sur le profil public (`reputation.history` existait déjà côté admin, jamais câblé côté public) — cf. `1-CheckList.md` A36
| Rév. t (2026-09-20, A37) — Bannières réduites, avatar/icône davantage superposés à la bannière (retour utilisateur sur le rendu réel) ; nouveau module `web/iconColor.js` (dépendance `pngjs`, pure JS) — couleur moyenne de l'icône en repli de bannière pour une guilde sans bannière/splash réel, rendue en `data:image/svg+xml` (CSP `img-src` étendue à `data:`, jamais `style-src`) faute de pouvoir passer par un `style` inline — cf. `1-CheckList.md` A37
| Rév. u (2026-09-20, A38) — En-tête global (`render.js:layout`) : barre de recherche + menu compte (`<details>`/`<summary>`, sans JS) sur toutes les pages, y compris celles jusque-là non authentifiées. `session.tryAuth` posé en hook global sur `publicRoutes` uniquement (scope encapsulé Fastify) — `layout`/`legalPage`/`notFoundPage`/`adminLayout` prennent désormais tous un `user`. Pages légales repassées en `Cache-Control: private` (leur contenu dépend maintenant de la session) — cf. `1-CheckList.md` A38
| Rév. v (2026-09-24, A44) — Nouveau logo (panda) dans `web/public/logo.png`/`favicon.png` ; `render.js` : le hash de contenu calculé au boot (`assetVersion`) couvre désormais `style.css`, `logo.png` et `favicon.png` (paramètre `?v=`), plus seulement la CSS — toute image statique référencée depuis `layout()` doit passer par ce mécanisme tant que `/static/*` reste `immutable, max-age=30j`. |
| Rév. w (2026-09-24, A45) — `db/pool.js` gagne `acquireProcessSingleton(pool, key, { logger, name, retryMs = 5000 })` : prend le verrou singleton de process sur un client dédié en **attendant** qu'il se libère (au lieu de `pg_try_advisory_lock` + `exit 0`), retourne ce client que l'appelant garde jusqu'au SIGTERM. `bot/main.js` et `jobs/main.js` l'utilisent — compatible avec un déploiement progressif (nouvelle instance démarrée avant l'arrêt de l'ancienne). |
...
| **Outputs** | `ownership.observe(source: 'gateway')` ; mise à jour `guilds.bot_present`, `member_count_cached`, `bot_role_position`, `icon_hash` (A35). |
...
> Covers : M2, M6, M13, M15, A5, A16, A17, A22, A35
